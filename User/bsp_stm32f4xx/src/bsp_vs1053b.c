...
#define VS1053_CS_0()	GPIOC->BSRRH = GPIO_Pin_2
#define VS1053_CS_1()	GPIOC->BSRRL = GPIO_Pin_2

#define VS1053_DS_0()	GPIOA->BSRRH = GPIO_Pin_5
#define VS1053_DS_1()	GPIOA->BSRRL = GPIO_Pin_5

#define VS1053_IS_BUSY()	((GPIOB->IDR & GPIO_Pin_8) == 0)
...
/*
*********************************************************************************************************
*	函 数 名: vs1053_WriteByte
*	功能说明: 向SPI1接口写一个字节
*	形    参: _ucByte ：数据字节.
*	返 回 值: 读到的数据.
*********************************************************************************************************
*/
uint8_t vs1053_WriteByte(uint8_t _ucByte)
{
	/* 等待发送区空 */
...
/*
*********************************************************************************************************
*	函 数 名: vs1053_ReadByte
*	功能说明: 从SPI1接口读一个字节
*	形    参: 无
*	返 回 值: 读到的数据.
*********************************************************************************************************
*/
uint8_t vs1053_ReadByte(void)
{
	return vs1053_WriteByte(DUMMY_BYTE);
}
...
void vs1053_WriteCmd(uint8_t _ucAddr, uint16_t _usData)
{
	...
	vs1053_WriteByte(VS_WRITE_COMMAND);	/* 发送vs1053的写命令 */
	vs1053_WriteByte(_ucAddr); 			/* 寄存器地址 */
	vs1053_WriteByte(_usData >> 8); 	/* 发送高8位 */
	vs1053_WriteByte(_usData);	 		/* 发送低8位 */
	VS1053_CS_1();
}
...
uint16_t vs1053_ReadReg(uint8_t _ucAddr)
{
	...
	VS1053_DS_1();
	VS1053_CS_0();
	vs1053_WriteByte(VS_READ_COMMAND);	/* 发送vs1053读命令 */
	vs1053_WriteByte(_ucAddr);			/* 发送地址 */
	usTemp = vs1053_ReadByte() << 8;	/* 读取高字节 */
	usTemp += vs1053_ReadByte();		/* 读取低字节 */
	VS1053_CS_1();
	return usTemp;
}
...
uint8_t vs1053_TestRam(void)
{
	...
	if (vs1053_WaitTimeOut())
	{
 	...
 	VS1053_DS_0();
	vs1053_WriteByte(0x4d);
	vs1053_WriteByte(0xea);
	vs1053_WriteByte(0x6d);
	vs1053_WriteByte(0x54);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	VS1053_DS_1();
...
void vs1053_TestSine(void)
{
	...
	*/
 	...
 	*/
    VS1053_DS_0();
	vs1053_WriteByte(0x53);
	vs1053_WriteByte(0xef);
	vs1053_WriteByte(0x6e);
	vs1053_WriteByte(0x24);	/* 0x24 or 0x44 */
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	...
	/* 退出正弦测试 */
    VS1053_DS_0();
	vs1053_WriteByte(0x45);
	vs1053_WriteByte(0x78);
	vs1053_WriteByte(0x69);
	vs1053_WriteByte(0x74);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
	vs1053_WriteByte(0x00);
